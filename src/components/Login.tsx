...
export const Login = ({ onLoginSuccess }: LoginProps) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Developer Bypass: Simply log in with any provided email/password
    setTimeout(() => {
      onLoginSuccess();
    }, 500);
  };
      ...
      <div className="w-full max-w-md p-8 bg-white/60 backdrop-blur-3xl border border-slate-200/50 rounded-[2rem] shadow-2xl shadow-purple-900/5 z-10 mx-4">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 mt-4 gradient-btn-tab hover:opacity-90 rounded-xl font-semibold shadow-lg transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group"
          >
            {isLoading ? (
