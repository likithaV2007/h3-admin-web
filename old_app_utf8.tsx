      ...
      if (student.location && student.location.coordinates) {
        ...
        if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
          ...
          if (!isInside && student.location.status !== 'Out of Bounds' && student.location.status !== 'On Leave') {
            stateChanged = true;
            const alertMsg = `≡ƒÜ¿ Geofence Violation: ${student.name} is outside the ${assignedFence.name} perimeter!`;
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            myLocMarker.bindPopup('<b>≡ƒôì Your Current Live Location</b><br/>Super Admin GPS Position').openPopup();
          },
          (error) => {
        ...
        polygonsList.forEach(polyObj => {
          ...
          poly.bindPopup(`
            <div style="font-family: sans-serif; padding: 2px; text-align: left;">
              ...
              <span style="font-size: 11px; font-weight: bold; color: ${gf.color};">≡ƒ¢í∩╕Å Geofence Active</span>
      ...
      studentPins.forEach((student, idx) => {
          ...
          html: `
            <div style="position: relative; display: flex; flex-direction: column; items-center: center;">
              <div style="background-color: #2563eb; color: white; padding: 2px 6px; border-radius: 8px; font-size: 10px; font-weight: bold; white-space: nowrap; border: 1.5px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.4);">
                ≡ƒæñ ${student.name.split(' ')[0]} (${student.batch || '2026'})
              </div>
        ...
        pinMarker.bindPopup(`
          <div style="font-family: sans-serif; padding: 3px;">
            ...
            <span style="font-size: 11px; font-weight: bold; color: #059669;">≡ƒôì ${student.location?.status || 'In Hostel'}</span>
          </div>
        `);
      });
  ...
  const [studentRequests, setStudentRequests] = useState<StudentRequest[]>([
    {
      ...
      details: 'Requesting Γé╣1,500 for registering in semester exam. The university portal closes on 25th May.',
      amount: 1500,
      status: 'Pending',
      date: '2026-05-18'
    },
    ...
    <div className={`min-h-screen transition-colors duration-300 flex ${darkMode ? 'dark bg-[#0b0f19] text-slate-100' : 'bg-[#f8fafc] text-slate-800'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto max-h-screen">
                ...
                <div className={`glass-panel rounded-2xl p-5 space-y-4 w-full ${activeRole === 'Admin' ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
                  ...
                  {(() => {
                    ...
                    const formatK = (val: number) => {
                      if (val >= 100000) return `Γé╣${(val / 100000).toFixed(0)}L`;
                      if (val >= 1000) return `Γé╣${(val / 1000).toFixed(0)}k`;
                      return `Γé╣${val}`;
                    };
                        ...
                        {(() => {
                          ...
                          expenses.slice(0, 10).forEach(exp => {
                            combinedLogs.push({
                              ...
                              role: 'Admin',
                              action: `Added expense: ${exp.title} (Γé╣${exp.amount.toLocaleString('en-IN')})`,
                        ...
                        {studentRequests.filter(r => r.status === 'Pending').length === 0 ? (
                          ...
                          studentRequests.filter(r => r.status === 'Pending').map(req => (
                            <div key={req.id} className="p-3 border border-slate-200/50 dark:border-slate-800/50 rounded-xl bg-slate-50/50 dark:bg-slate-900/30 flex flex-col gap-2">
                              ...
                              {req.amount && <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Requested: Γé╣{req.amount}</span>}
                    ...
                    <div className="glass-panel rounded-2xl p-5 space-y-4">
                      ...
                      <form onSubmit={handleStudentRequestSubmit} className="space-y-3">
                        ...
                        {newStudentRequestType === 'Fee Support' && (
                          <div>
                            <label className="text-[9px] font-bold text-slate-400 block mb-1">FEE AMOUNT (Γé╣)</label>
                            <input type="number" placeholder="Γé╣ Amount" value={newStudentRequestAmount} onChange={(e) => setNewStudentRequestAmount(e.target.value)} className="w-full text-xs p-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:outline-none" required />
                          </div>
                        )}
                        <div>
                      ...
                      <div className="space-y-3">
                        {studentRequests.map(req => (
                          <div key={req.id} className="p-3 border border-slate-100 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/30">
                            ...
                            {req.amount && <div className="text-[10px] font-bold text-slate-600 mt-1">Requested Amount: Γé╣{req.amount}</div>}
                          </div>
                        ))}
                      </div>
                    </div>
                          ...
                          <div className="glass-panel rounded-2xl p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                              ...
                              <div>
                                ...
                                <span className="font-bold text-slate-800 dark:text-slate-200">{selectedStudent.funding_percentage ? `${selectedStudent.funding_percentage}%` : 'N/A'} (Γé╣{selectedStudent.amount_approx || '0'})</span>
                              </div>
                              <div>
                ...
                <div className="lg:col-span-2 relative overflow-hidden bg-gradient-to-br from-blue-950 via-blue-700 to-white rounded-3xl p-7 text-white shadow-xl shadow-blue-950/20 flex flex-col justify-between min-h-[160px]">
                  ...
                  <div className="space-y-2 relative z-10">
                    ...
                    <div className="flex items-baseline gap-2 pt-2">
                      <span className="text-2xl font-black text-indigo-100">Γé╣</span>
                              ...
                              <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800/60 mt-1">
                                ...
                                <div className="flex items-center gap-3">
                                  <span className="text-lg font-black text-cyan-600 dark:text-cyan-400 font-mono">
                                    Γé╣ {item.amount.toLocaleString('en-IN')}
                                  </span>
                            ...
                            return (
                              <div key={cat} className="space-y-1">
                                <div className="flex justify-between text-xs font-bold">
                                  ...
                                  <span className="font-mono text-cyan-600">Γé╣{catTotal.toLocaleString('en-IN')} ({pct}%)</span>
                                </div>
                              ...
                              if (polygonCoords.length > 0) {
                                ...
                                poly.bindPopup(`
                                  <div style="font-family: sans-serif; padding: 2px;">
                                    <b style="font-size: 13px; color: #1e293b;">≡ƒôì ${result.display_name.split(',')[0]}</b><br/>
                                    <span style="font-size: 11px; color: #64748b;">${result.display_name}</span><br/>
                                    <span style="font-size: 11px; font-weight: bold; color: #ef4444;">≡ƒ¢í∩╕Å Automatic Location Geofence Active</span>
                                  </div>
                                `).openPopup();
                              }
                            }
                        ...
                        if (outFliers.length === 0) {
                          return (
                            <div className="flex flex-col items-center justify-center p-8 text-center bg-emerald-500/5 rounded-2xl border border-emerald-500/10">
                              <span className="text-2xl mb-1">≡ƒ¢í∩╕Å</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select className="w-full p-2.5 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 rounded-xl font-bold focus:outline-none focus:border-blue-500">
                        <option value="INR">Indian Rupee (Γé╣)</option>
        ...
        <footer className="mt-auto border-t border-slate-200/50 dark:border-slate-800/50 py-4 px-6 text-center text-[10px] text-slate-400 font-medium">
          Hope3 NGO Student Management System ΓÇó Powered by React.js & Tailwind CSS v4
        </footer>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5">
            ...
            <form onSubmit={handleCreateExpenseSubmit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[10px] font-extrabold text-slate-400 block mb-1 uppercase tracking-wider">AMOUNT (Γé╣)</label>
                  <input 
                    type="number" 
                    placeholder="1000" 
            ...
            <div className="flex-1 overflow-y-auto space-y-4 text-xs pr-1 scrollbar-none">
              <div className="text-center py-4 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-100 dark:border-slate-800/40">
                ...
                <span className="text-3xl font-black text-cyan-600 dark:text-cyan-400 font-mono">
                  Γé╣ {selectedExpense.amount.toLocaleString('en-IN')}
                </span>
              </div>
            ...
            >
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: 'pentagon', label: 'Pentagon (5-Sided)', icon: 'Γ¼ƒ' },
                    { id: 'hexagon', label: 'Hexagon (6-Sided)', icon: 'Γ¼ó' },
                    { id: 'polygon', label: 'Polygon (Custom)', icon: 'Γ¼º' }
                  ].map(sh => (
                    <button
                      key={sh.id}
                      type="button"
        ...
        <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col animate-fade-in">
          ...
          <div className="p-4 bg-slate-900 border-b border-slate-800 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                {mapType === 'hybrid' ? '≡ƒ¢░∩╕Å Satellite HD' : '≡ƒù║∩╕Å Google Streets'}
              </button>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-600 font-bold flex items-center gap-1">
                    Γ£ô Closed Polygon Loop ({pendingDrawnShape.coords.length} points)
                  </p>
                </div>
              </div>
              <button 
                  ...
                  if (pendingDrawnShape.layer && pendingDrawnShape.layer.bindPopup) {
                    pendingDrawnShape.layer.bindPopup(`
                      <div style="font-family: sans-serif; padding: 4px; text-align: left;">
                        <b style="font-size: 13px; color: #1e293b;">≡ƒôì ${nameToSave}</b><br/>
                        <span style="font-size: 11px; color: #64748b;">Allocated Batch: <b>${newZoneTargetBatch || 'ALL'}</b></span><br/>
                        <span style="font-size: 11px; font-weight: bold; color: #059669;">Γ£ô Boundary Closed & Active</span>
